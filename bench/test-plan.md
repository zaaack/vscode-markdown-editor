...
- [ ] An outline icon (3 horizontal lines) appears in the top toolbar, immediately to the right of the Find (🔍) icon.
- [ ] Clicking it opens Vditor's outline side panel.
- [ ] The outline lists every heading in the document.
- [ ] Clicking any heading in the outline scrolls the editor to that section.
- [ ] Clicking the outline icon again closes the panel.
- [ ] The "more" overflow menu (⋯) no longer contains "outline" — it was moved out of there.

## 2. Heading highlight setting (`f1d4619`)

**Setting:** `Markdown Editor: Highlight Headings` (default OFF).

- [ ] With the setting OFF: headings render with default Vditor styling. Nothing unusual.
- [ ] Toggle the setting ON and reopen the file.
- [ ] All headings (h1–h6) now have a tinted background band and a distinct foreground color.
- [ ] Switch VS Code to a light theme → the band remains readable (uses `--vscode-editor-selectionHighlightBackground` so it adapts).
- [ ] Switch to a dark theme → still readable.
- [ ] Switch to a high-contrast theme → still readable.
- [ ] Toggle the setting OFF and reopen — band disappears, headings revert.

## 3. Outline panel position setting (`f875852`)

**Setting:** `Markdown Editor: Outline Position` (enum `left` | `right`, default **`right`**).

- [ ] Default (`right`): clicking the outline icon opens the panel on the **right** side of the editor.
- [ ] Change setting to `left`, reload, reopen file.
- [ ] Now the outline opens on the **left** instead.
- [ ] Heading clicks still scroll to the right place from either side.
...
- [ ] Close all markdown editors. Open Cmd+Shift+P → *Developer: Show Running Extensions*.
- [ ] The extension should NOT be "activated" yet.
- [ ] Open a markdown file with the regular (text) editor → extension should still not be activated.
- [ ] Right-click the file → *Open With…* → Better Markdown Editor. Now it activates.
...
- [ ] Type a long paragraph at normal speed in the WYSIWYG view, then switch to the underlying text editor (split view).
- [ ] Updates should still appear in the text editor; latency between stopping typing and the text editor reflecting it should feel like ~quarter of a second, not instant.
- [ ] No visible "lag" while typing (the 250 ms is below typist-perceptible thresholds).
- [ ] No lost characters — what you typed in the WYSIWYG view should match what ends up in the text buffer.
...
- [ ] **Save** (Cmd+S) writes to the underlying file. Title shows `[edit]` marker before save, plain title after.
- [ ] **Find** (Cmd+F) opens the find widget. Search highlights matches.
